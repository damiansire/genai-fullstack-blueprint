...
const EXAMPLE_SPECS: Record<SupportedLanguage, string> = {
  typescript:
    'Create a type-safe HTTP retry function with exponential backoff and circuit breaker pattern',
  javascript: 'Build a debounced event emitter with wildcard subscriptions and once() support',
  python:
    'Implement a rate limiter using the token bucket algorithm with Redis-compatible interface',
...
export class CodeGenerator {
  ...
  async generate(): Promise<void> {
    ...
    try {
      ...
      const cacheRes = await this.aiOrchestrator.cacheContext(
        'spec.txt',
        'text/plain',
        this.spec(),
      );
      ...
      const res = await this.aiOrchestrator.generateCode(
        this.spec(),
        this.language(),
        cacheRes.cacheId,
      );
