...
- **Friday, May 1 (AI Model Selection Strategy)**: Defined the model routing strategy for the AI Gateway to optimize TTFT (Time To First Token), strict JSON schema compliance, and cost-efficiency.
  - **Implemented Practices in the AI Gateway (`packages/api`)**:
    - **Frontier Models (Generative UI & Orchestration) — roadmap, not implemented**: *Planned* use of **Claude 3.5 Sonnet** for generating Angular UI components and **Gemini 1.5 Flash** as the recursive orchestrator with *Context Caching*. As shipped, only Google plugins (`google-text-bison`, `gemini-image-gen`, `google-vision-ocr`) exist; there is **no Anthropic integration** in the codebase.
    - **Local SLMs (Edge Routing & Validation) — roadmap, not implemented**: *Planned* use of **Llama 3.1 (8B)** for zero-latency initial routing and **Phi-3.5 Mini** for asynchronous background validation (e.g., prompt injection detection) in Node.js *Worker Threads*. The shipped `aiSafetyFirewall` uses a basic keyword/substring heuristic only — no SLM is wired up.
...
- **Friday, May 1 (Advanced Agentic Patterns - Batch 2)**:
  ...
  - **Implemented Practices in the AI Gateway (`packages/api`):**
    ...
    - **Patrón 1 — JIT Tool Search (`src/application/useCases/tool-search.usecase.ts`, `src/api/routes/toolRoutes.ts`, `src/infrastructure/database/db.ts`)**: Replaced the static tool-schema injection in the System Prompt with a native `search_tools` discovery mechanism. A `tools` table is created in SQLite on startup. The LLM emits `tool_use: { name: "search_tools" }` → the agentic loop calls `ToolSearchUseCase` → schemas are injected **at the END** of the context window, keeping the static System Prompt prefix immutable so that, **once an Anthropic/OpenAI plugin is implemented**, the prefix would be eligible for provider-side prompt caching. *No Anthropic/OpenAI provider is integrated yet, so these cache hits do not occur today.* The figures below are **roadmap targets, not measured** — no benchmark exists in this repo: a hoped-for up to −31% TTFT and up to −85% token costs via prompt caching.
    ...
    - **Patrón 5 — AsyncLocalStorage Extendido para Árboles Agenticos (`src/core/async-context.ts`)**: Extended `RequestContext` from a flat `{ traceId }` to a full agentic span tree: `{ traceId, spanId, parentSpanId?, depth, toolCallStack[] }`. Added `createRootContext()` (called once per HTTP request) and `createChildContext(toolName, parentCtx)` (called inside Worker Thread handlers). The child context is forwarded to the tool worker in the task message payload (`CPUWorkerService.executeTool(..., agentContext)`); the worker reads it and echoes it back on its reply, so the parent can stitch the span tree for an OTLP collector (Jaeger, Grafana Tempo) without prop-drilling or function-signature pollution. *Note*: `AsyncLocalStorage` is **not** propagated into the worker isolate — only the serialized context object crosses the boundary.
    ...
    - **Patrón 2 — Servidor Nativo MCP (`src/infrastructure/mcp/`)**: Implemented a zero-dependency Model Context Protocol server exposing the Gateway's capabilities via two transports. **stdio transport** (`npm run mcp:stdio`): reads newline-delimited JSON-RPC 2.0 from stdin/stdout using `node:readline` — compatible with Claude Desktop, Cursor, Zed, and Continue.dev. **SSE transport** (`GET /mcp/sse` + `POST /mcp/message`): reuses the existing `text/event-stream` infrastructure with an in-memory `Map` of session connections. MCP capabilities: `tools/list` (reads from SQLite Tool Registry, Patrón 1), `tools/call` (delegates to Worker Pool), `resources/list|read` (exposes models, logs, tools registry), `prompts/list|get` (reusable interaction templates). Both transports seed `AsyncLocalStorage` root contexts per message (fully Patrón 5 compatible).
      - *Reference*: [MCP Spec v2024-11-05](https://modelcontextprotocol.io/specification/2024-11-05) · [Node.js readline API](https://nodejs.org/docs/latest/api/readline.html)
...
- **Saturday, May 2 (Enterprise Grade Gateway Features)**: Implemented production-ready scaling, governance, and observability features to transition the scaffold into an Enterprise AI Gateway.
  - **Implemented Practices in the AI Gateway (`packages/api`)**:
    ...
    - **Semantic Caching with sqlite-vec (P2)**: `semanticCache.lookup()` is wired into `InvokeModelUseCase` with a two-tier design: Tier 1 semantic match (L2 distance over int8-quantized embeddings via `sqlite-vec`) falling back to Tier 2 exact SHA-256 hash match. **Caveat**: `sqlite-vec` is **not bundled** and must be supplied as an external SQLite extension; when it cannot be loaded the Tier 1 path is disabled and only the Tier 2 exact-match path runs. The "0ms / 0 token" benefit therefore applies to **identical** repeat queries, not paraphrases, unless the extension is installed.
    - **RBAC per Model Tiers (P3)**: Created an Express `rbacModelMiddleware` using the *Fail-Fast* pattern. It prevents unprivileged users (e.g., `free` tier) from waking up premium models (e.g., `gemini-1.5-pro` or `gemini-image-gen`) by matching the required tier from an internal map against `req.user.tier`. **Caveat (as shipped)**: there is no JWT in the codebase and nothing populates `req.user.tier`, so every request currently resolves to the most restrictive (`free`) tier. Populating the tier from a real identity source is a prerequisite for the tier routing to actually discriminate.

- **Saturday, May 2 (Autonomous Agentic & RAG Modernization)**: 
  - **Implemented Practices in the AI Gateway (`packages/api`)**:
    - **Autonomous Code Generator Agent (P1)**: Connected the `CodeGenerationUseCase` to `modelFactory` to use `google-text-bison` (or `gemini-1.5-pro` logic). Eliminated static/deterministic templates in favor of an iterative RCI (Recursive Critique and Improvement) loop: the LLM generates code, the built-in static analyzers grade the complexity and smells, and if the quality falls below `QUALITY_THRESHOLD`, the LLM auto-corrects the code dynamically.
    - **Gemini Context Caching API (P2)**: Designed the `ContextCacheUseCase` (`/api/domain/context-cache`) to handle high-volume text/document contexts (RAG). Integrated `gemini_context_cache` table directly in SQLite to keep track of persistent cache hashes (e.g., keeping track of `cacheId`), drastically cutting RAG costs when RCI loops are executed repeatedly on the same specification.
    - **Rate Limit State Persistence (P3)**: Eliminated `InMemoryTokenStore` volatility by migrating the Token-based Rate Limiting subsystem to SQLite via `SqliteTokenStore`. This allows persistent cross-deployment Token budgets to be enforced strictly using the new `rate_limit_tokens` table.

- **Saturday, May 2 (Enterprise Platform Capabilities & B2B SaaS Readiness)**: 
  - **Implemented Practices in the AI Gateway (`packages/api`)**:
    - **Guardrails & AI Safety Firewall (P1)**: Implemented `aiSafetyFirewall` middleware in `src/api/middleware/ai-safety.middleware.ts` to scan incoming HTTP requests before reaching LLM plugins. Integrated PII masking using regex heuristics to sanitize emails and sensitive data. The toxicity/prompt-injection filter is a **basic keyword/substring heuristic** (trivially evaded, prone to false positives); a local Phi-3.5 Background SLM is a roadmap item, not implemented.
    - **Integrated RAG & Vector Ingestion (P1)**: Created `RAGIngestUseCase` (`src/application/useCases/rag-ingest.usecase.ts`) to handle semantic chunking and ingestion of private documents into the `sqlite-vec` index (`rag_vectors` table). This lays the foundation for enterprise "Chat with Data" capabilities.
    - **Multi-Tenant Architecture (P2) — design, not implemented**: The domain boundary is *designed* to carry a `tenantId` so that vector databases and chat histories could be partitioned per B2B customer. **As shipped there is no tenant isolation**: no JWT verification, no per-tenant binding, and no data segregation by `tenant_id`; `RAGIngestUseCase` itself now returns `501 Not Implemented`. Do **not** rely on this for any compliance (GDPR/HIPAA) claim — implementing the per-tenant binding and query scoping is a prerequisite.
    - **Prompt Playground (Modo Administrador) (P1)**: Created `prompts` table in SQLite and `/api/admin/prompts` endpoints to externalize System Prompts from hardcoded Node.js files into the database. This allows real-time tuning of agent behavior without server restarts.
    - **Token Dashboard API (P1)**: Exposed `/api/user/quota` reading dynamically from `rate_limit_tokens` table. Allows frontend to display real-time token consumption vs monthly allocations.
    - **Chat History & Session Management (P1)**: Created `sessions` and `messages` tables with strict cascading foreign keys and multi-tenant isolation. Exposed via `/api/sessions`. Prepares the foundation for Sliding Window context management.
