...
- The test suites of both workspaces. Backend: the **`node:test` suite (<!--METRICS:BACKEND-->114<!--/METRICS:BACKEND--> tests)**, including **HTTP integration tests** that boot the real gateway on an ephemeral port and drive the whole chain end-to-end: health probes, the fail-closed auth boundary, and `auth → rate-limit → token-limit → safety → handler`. Frontend: the **Vitest suite (<!--METRICS:FRONTEND-->44<!--/METRICS:FRONTEND--> tests)** covering the client's domain layer: SSE parsing/cancellation in `AiStreamService`, frame-paced streaming render, JSON-Schema-to-form generation, the API-key interceptor boundary, and the route table. Component-template rendering and browser E2E are not in CI (see [ADR 0004](./docs/adr/0004-scaffold-boundaries.md)). Both counts are generated by `scripts/update-metrics.mjs`; CI runs it with `--check` and fails on any drift, so a stale or hand-typed number cannot ship.
- The **Angular production bundle** compiles.
- The **entire Docker Compose stack** (API + client) builds from its Dockerfiles and **boots to healthy**, with its live endpoints smoke-tested. That smoke test is a real, runnable script: [`scripts/smoke.sh`](./scripts/smoke.sh) brings up the stack (`docker compose up -d --build --wait`) and curls every declared endpoint, failing if any is unhealthy; CI runs that same script on every push, and you can run it locally with `bash scripts/smoke.sh`.
...
> **🏗️ Architecture:** Patterns and API reference live in `REGISTRY.md`. The reasoning behind the load-bearing decisions (why an Express gateway, what happens when Gemini fails or rate-limits, the SSE streaming strategy, and the declared limits of this scaffold) is recorded in [`docs/adr/`](./docs/adr/README.md).
...
- Extract text from images (accuracy depends on the upstream Vision model and input quality)
- Multi-language support (whatever the upstream Vision API recognizes)
...
- Multiple aspect ratios (Square, Portrait, Landscape, Widescreen, etc.)
...
- **Low-Cost Caching**: The **exact-match** cache (SHA-256 of the prompt) serves identical repeat queries locally, with no provider call and no token cost. The **semantic** (paraphrase-tolerant) path requires the optional `sqlite-vec` extension and is off by default; without it, paraphrased queries are not cache hits.
...
- **[Architecture Decision Records (docs/adr/)](./docs/adr/README.md)** - Why the gateway exists, how provider failure/rate-limiting is handled, the streaming strategy, and the declared limits of the scaffold.
- **[Master Architectural Registry (REGISTRY.md)](./REGISTRY.md)** - API references, setup workflow, component structures, and Node.js native patterns.
- **[SECURITY.md](./SECURITY.md)** - Scoped security review. **[docs/load-test.md](./docs/load-test.md)** - Measured load numbers. **[docs/deployment.md](./docs/deployment.md)** - Deployment guide.
